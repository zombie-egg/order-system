...
1. **架构设计**：已完成并按人工履约修订。
2. **创建项目**：已完成。工程骨架、健康端点、占位应用、质量工具、CI 和文档。
3. **Backend**：已完成商品、计价、订单、支付状态机、KDS 工单、人工审核、审计和对账。
4. **Frontend**：已完成 Kiosk、KDS、Admin 完整业务流。
5. **UI 优化**：已完成设备分辨率、触控、无障碍、荷兰语顾客流和异常恢复的软件边界；仍需 15/21 英寸真机现场验收。
6. **Adyen 实验室与商户接入**。
...
- 三个 Frontend 的业务页面与 Phase 5 商用设备 UI 已实现；仍需真机、支付终端、打印机和 Windows 锁定环境现场验收。
- 真实 Adyen/Stripe/SumUp 凭证、Terminal API 网络调用、Webhook 与结算文件；需后续实验室和商户接入。
- 云端多门店同步、OTA、Windows Service 安装与 Kiosk 锁定。
- 自动饮品机、Serial、MQTT、Modbus 或硬件模拟器。
- 未经人工审核自动处理无法履约订单或自动退款。

当前软件阶段仍只提供开发用 Mock 支付，不宣称已完成正式 PSP、税务发票、Windows 现场部署或设备认证。
